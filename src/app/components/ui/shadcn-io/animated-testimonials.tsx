...
export const AnimatedTestimonials = ({
  ...
  // Precompute rotations for each testimonial to avoid hydration mismatch
  const rotations = useMemo(() => 
    testimonials.map((_, index) => (index % 3 - 1) * 5), // -5, 0, 5 degrees
    [testimonials]
  );
    ...
    <div className={cn("mx-auto max-w-sm px-4 py-20 font-sans antialiased md:max-w-4xl md:px-8 lg:px-12", className)}>
      <div className="relative grid grid-cols-1 gap-20 md:grid-cols-2">
        ...
        <div className="flex flex-col justify-between py-4">
          ...
          >
            <h3 className="text-2xl font-medium bg-linear-to-r from-[#deffcb] to-white bg-clip-text text-transparent dark:text-white">
              {testimonials[active].name}
            </h3>
            <p className="text-gradient-hero">
